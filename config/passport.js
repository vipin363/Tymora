  ...
  new GoogleStrategy(
    ...
    async (req, accessToken, refreshToken, profile, done) => {
      try {
        ...
        console.log(
          `[Google OAuth] Callback received | mode=${isRegister ? 'register' : 'login'} | email=${email}`
        );
        ...
        if (!isRegister) {
          if (!user) {
            console.log(
              `[Google OAuth] Login failed – user not found: ${email}`
            );
            return done(null, false, { message: 'User not found' });
          }
          if (user.isBlocked) {
            console.log(
              `[Google OAuth] Login failed – account blocked: ${email}`
            );
        ...
        if (user) {
          console.log(
            `[Google OAuth] Register failed – user already exists: ${email}`
          );
        ...
        console.log(
          `[Google OAuth] Referral code recovered: ${rawRefCode || 'none'}`
        );
        ...
        if (rawRefCode) {
          ...
          if (!referrer) {
            console.log(
              `[Google OAuth] Referral code invalid – no matching user: ${rawRefCode}`
            );
          } else {
            // Self-referral check
            if (referrer.email === email) {
              console.log(
                `[Google OAuth] Self-referral attempt detected - skipping.`
              );
            } else {
              ...
              if (alreadyReferred) {
                console.log(
                  `[Google OAuth] Referral already used by this email – skipping.`
                );
              } else {
                referrerId = referrer._id;
                referralCodeUsed = rawRefCode;
                console.log(
                  `[Google OAuth] Referral valid – referrer: ${referrer.email}`
                );
              }
            }
          }
        ...
        const prefix = (profile.displayName || 'USER')
          .replace(/[^A-Z0-9]/gi, '')
          .toUpperCase()
          .slice(0, 4);
        let newCode, exists;
        ...
        console.log(
          `[Google OAuth] New user created: ${email} | referralCode=${newCode}`
        );
        ...
        if (referrerId && referralCodeUsed) {
          ...
          if (settings?.referralProgramEnabled !== false) {
            ...
            console.log(
              `[Google OAuth] Referral record created – Pending rewards queued.`
            );
          } else {
            console.log(
              `[Google OAuth] Referral program disabled – no record created.`
            );
          }
        }
